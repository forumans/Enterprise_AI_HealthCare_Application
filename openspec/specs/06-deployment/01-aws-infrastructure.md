# Prompt: AWS Deployment Architecture (Lambda + API Gateway + SAM)
...
**Critical:** The `/api/*` behavior must use the `AllViewerExceptHostHeader` origin request policy (ID: `b689b0a8-53d0-40ab-baf2-68738e2966ac`). Without it, CloudFront forwards the viewer's `Host` header (the CloudFront domain) to API Gateway, which rejects it with 403. CloudFront's custom error rules then return `index.html` instead of JSON — a silent failure that is hard to diagnose.

CloudFront custom error rules: `403 → /index.html` and `404 → /index.html` (SPA fallback for React Router).
