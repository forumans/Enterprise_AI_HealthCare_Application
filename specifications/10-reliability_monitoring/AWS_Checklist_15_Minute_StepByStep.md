# AWS Fast-Track Bootstrap: Lambda + API Gateway (Step-by-Step)

## Purpose
A practical checklist to stand up this Healthcare SaaS application on AWS from scratch. Follow this in order. A contributor with AWS CLI access and the repo cloned should complete first deploy in ~20 minutes.
...
# Create .env.local for local testing (not needed for S3 deploy)
echo "VITE_API_BASE_URL=https://<api-id>.execute-api.us-east-1.amazonaws.com" > .env.local
...
- Origin request policy: **`AllViewerExceptHostHeader`** (critical — without this, API Gateway returns 403)
